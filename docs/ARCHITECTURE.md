...
| `/api/passwords.create` | `create_password` | `site` (HTTP/S URL), `username`, optional `password` (empty for passkey-only logins), `name` (display name, up to 200 characters), `category` and `notes` |
...
`LoginFactors` adds exact-origin discovery (`passwords.findLogin` / `find_login`), encrypted TOTP configuration (`passwords.setTotp` / `configure_totp`), current-code generation (`passwords.totp` / `get_totp`), passkey enrollment storage (`passwords.setPasskey` / `save_passkey`), temporary passkey checkout (`passwords.passkey` / `get_passkey`) and release (`passwords.releasePasskey` / `release_passkey`). Configure operations require the entry's current version and write permissions. Other operations require credential read permissions. The passkey checkout/release mutate internal counter and lease bookkeeping even for read-scoped callers. Guests, suspended accounts and other workspaces remain excluded.

A login's optional `loginUrl` binds factor disclosure to the exact HTTPS origin where authentication happens; it falls back to `site`. This handles an application like Gmail whose login origin differs from its application URL. Discovery returns metadata and method availability; `info`, list, create and update never return TOTP seeds, private passkey material or leases. OTP responses contain only the current code and its expiry. Factors and lease IDs live inside the existing authenticated encrypted payload, so no database migration is required. Ordinary edits preserve them. Passwordless entries can be created with an empty password before enrolling the passkey.

TOTP accepts a Base32 setup key or an `otpauth://totp` URI, supports SHA1/SHA256/SHA512, six or eight digits and periods from 15 to 120 seconds. Codes follow [RFC 6238](https://www.rfc-editor.org/rfc/rfc6238) and are generated on the server. Configure the factor on the target service and confirm it there after saving the setup secret in Outline; importing a seed alone does not enable the factor on the service.

The agent bridge is `plugins/passwords/agent/browser.mjs`: `createVaultClient`, `findLogin`, `beginPasskeyRegistration`, `withPasskey`, and `getTotp`. It takes a Playwright Chromium page and uses the [Chrome DevTools WebAuthn protocol](https://chromedevtools.github.io/devtools-protocol/tot/WebAuthn/). Enrollment happens on the target service while already authenticated, using a new resident ES256 software authenticator. The helper saves the PKCS#8 private key, RP ID, credential ID, user handle, backup flags and initial signature counter directly to the encrypted vault, then removes the authenticator. If saving fails, keep the browser and enrollment handle alive and retry before discarding the new key. This is not an export mechanism for hardware, iCloud or arbitrary password-manager passkeys, nor does Outline become a system passkey provider.

Each use checks out a two-minute exclusive lease, atomically reserving the next 32 signature counter values before exporting the key. `withPasskey` enforces a shorter deadline and assertion budget, removes the authenticator in `finally`, then releases the lease. An abandoned lease expires; the next browser starts beyond the reserved counter range. This prevents normal concurrent usage and counter rollback after crashes. Clients must not retain or reuse exports beyond the lease, bypass the assertion budget or use the private key outside the helper. A server-side lease cannot revoke an already exported private key. Metadata edits use versions incremented by factor configuration and lease operations, preventing lost updates.

These are software credentials held by the agent, not hardware-backed proof of human presence. Keeping a password and its second factor in one vault means a vault reader can use both; it does not preserve independent custody of the factors. Site policies, CAPTCHA, unsupported WebAuthn features and physical-device requirements can still prevent unattended login. Removing a factor from Outline does not revoke it at its relying party.

Run the real browser regression with `yarn playwright install chromium` then `node --test plugins/passwords/agent/browser.test.mjs`; `CHROME_PATH` optionally selects an installed Chromium executable. The test registers against a locally intercepted `.test` origin, verifies real WebAuthn signatures, closes the registering context and authenticates in two fresh contexts. The production workflow runs this check before publishing an image. API tests cover encrypted storage, seed/key masking, origin mismatch, scope/role/workspace isolation, counter reservation and partial-edit preservation.

If enrollment persistence fails with a stale version, keep the registration handle open, refresh the entry metadata, and retry `save(freshVersion)`. Do not repeat registration or close the browser before saving or explicitly cancelling.
